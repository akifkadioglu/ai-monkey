/**
 * openai.js - OpenAI provider for AI Monkey
 *
 * Sends chat completion requests to the OpenAI API and parses
...
export class OpenAIProvider extends LLMProvider {
  /**
   * Generate code via the OpenAI chat completions API.
   *
  ...
  async generate(systemPrompt, userMessage) {
    ...
    if (!data.choices?.length || !data.choices[0].message?.content) {
      throw new Error('OpenAI response missing choices or message content');
    }
