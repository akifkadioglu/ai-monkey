/**
 * groq.js - Groq provider for AI Monkey
 *
 * Sends chat completion requests to the Groq API (OpenAI-compatible)
...
export class GroqProvider extends LLMProvider {
  /**
   * Generate code via the Groq chat completions API.
   *
  ...
  async generate(systemPrompt, userMessage) {
    ...
    if (!data.choices?.length || !data.choices[0].message?.content) {
      throw new Error('Groq response missing choices or message content');
    }
