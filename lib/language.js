 ...
 * @returns {{ isEnglish: boolean, confidence: number }}
 */
function detectLanguage(text) {
  ...
  if (hasNonLatinChars(text)) {
    return { isEnglish: false, confidence: 0.9 };
  }

  // Check English word frequency for Latin-script languages
  const score = englishScore(text);
  if (score > 0.15) {
    return { isEnglish: true, confidence: score };
  }

  return { isEnglish: false, confidence: 1 - score };
}

export { detectLanguage, hasNonLatinChars, englishScore };
